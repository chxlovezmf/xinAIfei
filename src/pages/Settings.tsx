...
import { Sun, Moon, Database, Download, Upload, Trash2, Palette, Info } from 'lucide-react';
...
import { getAllTransactions, getAllNotes, deleteTransaction, deleteNote } from '../db/database';
...
export default function Settings() {
  ...
  const handleExport = async () => {
    const [txs, notes, cats] = await Promise.all([
      getAllTransactions(),
      getAllNotes(),
      getCategories(),
    ]);
    ...
    <PageTransition>
      <div className="page-container">
        ...
        <motion.div
          ...
          className="card mb-3 flex items-center justify-between"
        >
        ...
        >
          ...
          {importStatus && (
            <p className="mt-2 text-xs text-gray-500">{importStatus}</p>
          )}
        </motion.div>
        ...
        >
          ...
          <div>
            <p className="text-sm font-medium text-gray-800 dark:text-gray-200">关于记一记</p>
            <p className="text-xs text-gray-400">版本 1.0 · PWA 应用 · 数据全在本地</p>
          </div>
        </motion.div>
      </div>
    </PageTransition>
